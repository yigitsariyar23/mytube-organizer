...
html.body = html.body.replace('<head>', `<head><base href="/dashboard/"><meta name="theme-color" content="#18191c"><meta name="apple-mobile-web-app-capable" content="yes"><meta name="apple-mobile-web-app-title" content="MyTube"><link rel="manifest" href="/manifest.webmanifest"><link rel="apple-touch-icon" href="/icons/icon128.png">`)
  ...
  .replace('<script type="module" src="dashboard.js"></script>', `<div id="mobileSaveModal" class="modal" hidden><section class="modal-box" role="dialog" aria-modal="true" aria-labelledby="mobileSaveTitle"><h2 id="mobileSaveTitle">Save from YouTube</h2><p class="mobile-save-hint">Copy a video or playlist link from YouTube, then paste it here.</p><label class="field-label" for="mobileUrl">YouTube link</label><input id="mobileUrl" class="field-input" type="url" placeholder="https://youtu.be/…" autocomplete="off"><label class="field-label" for="mobileList">Save video to</label><select id="mobileList" class="field-input"></select><p class="mobile-save-hint">Playlist imports create a new list after review. Public/unlisted playlists need your YouTube API key.</p><p id="mobileSaveStatus" role="status"></p><div class="mobile-save-actions"><button id="mobileSaveVideo" class="btn btn-primary" type="button">Save video</button><button id="mobileImportPlaylist" class="btn btn-ghost" type="button">Import playlist</button><button id="mobileSaveCancel" class="btn btn-text" type="button">Cancel</button></div></section></div><script type="module" src="/mobile/bootstrap.js"></script>`);
